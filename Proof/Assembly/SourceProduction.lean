import Proof.Assembly.FamilyProduction
...
noncomputable abbrev ProducedSourceSpec (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (sources : EightSources) {gamma : Real} (p : Parameters sources gamma)
...
∃ (refillCost : Nat),
∃ (firstCost : Nat),
∃ (refillCode : FamilyCode packet row U),
∃ (firstCode : FamilyCode packet row (U+1)),
let refillCycle := refillCode.cached
let firstCycle := firstCode.cached
...
∃ (_hrefill : (∀ j<entries.length,∀ Z,Step family (fuel j) (H j) (A j) (outH j) (outA j Z)  → 
   FamilyCode.Prepared refillCode (ds (j+1)) refillCost
    (outH j) (H (j+1)) (fun i=>ZeroPadding.pad (reserve i) (outA j Z i)) (padded (j+1)))),
∃ (_hfirst : FamilyCode.Prepared firstCode (ds 0) firstCost originalH
...
noncomputable def producedSourceProjection (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (sources : EightSources) {gamma : Real} (p : Parameters sources gamma)
 (k den r scratch : Nat) (clock : OrdinaryClock (fun n=>n^(k+2)))
 (n : Nat) (x : BitInput n)
 (oracle : BooleanCircuit ((SelectedRecoveryIntegration.outer sources k clock).result.pcp.nativeWidth n))
 (bits : List Bool)
 ...
 (globalH globalHnext : Fin (ControllerSelectedContinuation.bodyTapes sources p k r scratch)→Nat)
 (globalA globalAfter : Fin (ControllerSelectedContinuation.bodyTapes sources p k r scratch)→List Bool)
 (b siteFuel : Nat) (supplied : ProducedSourceSpec selector packets rows compiler sources p k den r scratch clock n x oracle bits site mode ph ci globalH globalHnext globalA globalAfter b siteFuel) :
 PCJ38fbfed565f64139_Physical.CachedSourceSpec selector compiler sources p k den r scratch clock n x oracle bits site mode ph ci globalH globalHnext globalA globalAfter b siteFuel := by
 ...
 exact ⟨order,horder,L,target,a,rowWork (rows (decompositionOf sources) a).privateWork,
   (rows (decompositionOf sources) a).program,prepT,hsize,dflt,ds,xs,S,R,B,rowWidth,total,denominator,D,cap,logSize,resetSize,coefficient,old,phasePrefix,entries,hlen,hxs,hcoeff,hnative,U,slots,hs,enc,he,app,ha,H,A,reserveSize,hw,hfit,hH,hA,hc,hold,hl,hr,heH,haH,hfields,familyCost,refillCost,firstCost,refillCode.cached,firstCode.cached,refillSource,firstSource,whole,hwhole,counterReserve,hcost,refills,firstReady,hfinalH,hfinalA,hcode,hbound⟩
...
noncomputable abbrev ProducedPhase (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (sources : EightSources) {gamma : Real} (p : Parameters sources gamma)
 ...
 ∃ (b : Nat)
   ...
   (PCPPQueryCachedBounds.capacity (CloseoutLanguage.selectedPCPP sources) ((req sources k clock x oracle).circuit.size+(req sources k clock x oracle).arity)) [] i)
    (_s_source : ∀ ci : Fin (2^((CloseoutLanguage.selectedPCPP sources).output (req sources k clock x oracle)).clauseBits),
      ProducedSourceSpec selector packets rows compiler sources p k den r scratch clock n x oracle bits site mode ph ci
...
noncomputable def producedPhaseProjection (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (sources : EightSources) {gamma : Real} (p : Parameters sources gamma)
 (k den r scratch : Nat) (clock : OrdinaryClock (fun n => n^(k+2))) (n : Nat) (x : BitInput n)
 (oracle : BooleanCircuit ((SelectedRecoveryIntegration.outer sources k clock).result.pcp.nativeWidth n))
 (bits : List Bool)
 (site : Bool → CloseoutRowsOriginalSchedule.Phase → Σ states,
 ...
 (hin : Fin (PCJda54a286946142d3_BranchPhases.tapes sources p k r scratch) → Nat)
 (tin : Fin (PCJda54a286946142d3_BranchPhases.tapes sources p k r scratch) → List Bool)
 (fuel width : Nat) (supplied : ProducedPhase selector packets rows compiler sources p k den r scratch clock n x oracle bits site mode ph hin tin fuel width) : PCJ38fbfed565f64139_Physical.CachedPhase selector compiler sources p k den r scratch clock n x oracle bits site mode ph hin tin fuel width := by
 ...
 have sourcesReady := fun ci => producedSourceProjection selector packets rows compiler sources p k den r scratch clock n x oracle bits site mode ph ci
...
noncomputable def producedBuildPhase (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (tables : TableCertificate) (semantics : Certificate) (buildSource : SourceBuilder) (sources : EightSources) {gamma : Real} (p : Parameters sources gamma)
 (k den r scratch : Nat) (clock : OrdinaryClock (fun n => n^(k+2))) (n : Nat) (x : BitInput n)
 (oracle : BooleanCircuit ((SelectedRecoveryIntegration.outer sources k clock).result.pcp.nativeWidth n))
 (bits : List Bool)
 (site : Bool → CloseoutRowsOriginalSchedule.Phase → Σ states,
 ...
 (hin : Fin (PCJda54a286946142d3_BranchPhases.tapes sources p k r scratch) → Nat)
 (tin : Fin (PCJda54a286946142d3_BranchPhases.tapes sources p k r scratch) → List Bool)
 (fuel width : Nat) (supplied : ProducedPhase selector packets rows compiler sources p k den r scratch clock n x oracle bits site mode ph hin tin fuel width) :
 ...
 PCJ38fbfed565f64139_Physical.cachedBuildPhase selector compiler tables semantics buildSource sources p k den r scratch clock n x oracle bits site mode ph hin tin fuel width
   (producedPhaseProjection selector packets rows compiler sources p k den r scratch clock n x oracle bits site mode ph hin tin fuel width supplied)
...
noncomputable abbrev ProducedSelected (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (tables : TableCertificate) (semantics : Certificate) (buildSource : SourceBuilder) (sources : EightSources) {gamma : Real} (p : Parameters sources gamma)
...
noncomputable def producedSelectedProjection (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (tables : TableCertificate) (semantics : Certificate) (buildSource : SourceBuilder) (sources : EightSources) {gamma : Real} (p : Parameters sources gamma)
 (den : Nat) (hden : 0<den) (k r scratch n : Nat) (x : BitInput n) (bits : List Bool)
 (hp : P1Independent.CappedLegalAdmission.passed sources p
  (ControllerCappedSelected.reference den hden k (PolynomialClock.ordinaryClock k))
  (WorkspaceSelectedAdmission.coldCutoff sources) n x bits = true)
 (site : Bool → Phase → Σ states, Machine (ControllerSelectedContinuation.bodyTapes sources p k r scratch) states)
 (remainingFuel : Nat) (width : Phase → Nat) (supplied : (ProducedSelected selector packets rows compiler tables semantics buildSource) sources p den hden k r scratch n x bits hp site remainingFuel width) : (PCJ38fbfed565f64139_Physical.CachedSelected selector compiler tables semantics buildSource) sources p den hden k r scratch n x bits hp site remainingFuel width := by
 rcases supplied with ⟨cost,penaltySpec,momentSpec,clauseSpec,penalty_kept,moment_kept,threshold,head,words,blank,fits⟩
 let penalty := (producedBuildPhase selector packets rows compiler tables semantics buildSource) sources p k den r scratch (PolynomialClock.ordinaryClock k) n x (C10TotalDecode.oracleOf sources k (PolynomialClock.ordinaryClock k) p.degree n bits) bits site (PCJ374c44bb8b7f47d9_.S.mode sources p den hden k r scratch n x bits hp) .penalty (PCJ374c44bb8b7f47d9_.S.heads sources p k r scratch) (PCJ374c44bb8b7f47d9_.S.bank sources p den hden k r scratch n x bits hp) (cost .penalty) (width .penalty) penaltySpec
 let moment := (producedBuildPhase selector packets rows compiler tables semantics buildSource) sources p k den r scratch (PolynomialClock.ordinaryClock k) n x (C10TotalDecode.oracleOf sources k (PolynomialClock.ordinaryClock k) p.degree n bits) bits site (PCJ374c44bb8b7f47d9_.S.mode sources p den hden k r scratch n x bits hp) .moment penalty.realize.heads penalty.realize.exit (cost .moment) (width .moment) momentSpec
 let clause := (producedBuildPhase selector packets rows compiler tables semantics buildSource) sources p k den r scratch (PolynomialClock.ordinaryClock k) n x (C10TotalDecode.oracleOf sources k (PolynomialClock.ordinaryClock k) p.degree n bits) bits site (PCJ374c44bb8b7f47d9_.S.mode sources p den hden k r scratch n x bits hp) .clause moment.realize.heads moment.realize.exit (cost .clause) (width .clause) clauseSpec
 exact ⟨cost,((producedPhaseProjection selector packets rows compiler) sources p k den r scratch (PolynomialClock.ordinaryClock k) n x (C10TotalDecode.oracleOf sources k (PolynomialClock.ordinaryClock k) p.degree n bits) bits site (PCJ374c44bb8b7f47d9_.S.mode sources p den hden k r scratch n x bits hp) .penalty (PCJ374c44bb8b7f47d9_.S.heads sources p k r scratch) (PCJ374c44bb8b7f47d9_.S.bank sources p den hden k r scratch n x bits hp) (cost .penalty) (width .penalty) penaltySpec),((producedPhaseProjection selector packets rows compiler) sources p k den r scratch (PolynomialClock.ordinaryClock k) n x (C10TotalDecode.oracleOf sources k (PolynomialClock.ordinaryClock k) p.degree n bits) bits site (PCJ374c44bb8b7f47d9_.S.mode sources p den hden k r scratch n x bits hp) .moment penalty.realize.heads penalty.realize.exit (cost .moment) (width .moment) momentSpec),((producedPhaseProjection selector packets rows compiler) sources p k den r scratch (PolynomialClock.ordinaryClock k) n x (C10TotalDecode.oracleOf sources k (PolynomialClock.ordinaryClock k) p.degree n bits) bits site (PCJ374c44bb8b7f47d9_.S.mode sources p den hden k r scratch n x bits hp) .clause moment.realize.heads moment.realize.exit (cost .clause) (width .clause) clauseSpec),penalty_kept,moment_kept,threshold,head,words,blank,fits⟩

noncomputable abbrev ProducedRecipe (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (tables : TableCertificate) (semantics : Certificate) : Prop :=
...
theorem producedRecipeProjection (selector : CyclicChoice.Laws) (packets : PacketLibrary selector) (rows : RowLibrary selector) (compiler : Packets.CompilerLaws) (tables : TableCertificate) (semantics : Certificate) (physical : ProducedRecipe selector packets rows compiler tables semantics) : PCJ38fbfed565f64139_Physical.CachedRecipe selector compiler tables semantics := by
 classical
 intro buildSource
 ...
 exact producedSelectedProjection selector packets rows compiler tables semantics buildSource sources p
...
namespace PCJd4d1d9d7d1fa4313_Production
open NearCubicWires NearCubicWires.ExtIncidence PCJ9eff70d512234a4c_Fixed
noncomputable def SourceAssembly (selector : CyclicChoice.Laws) (compiler : Packets.CompilerLaws)
    (tables : TableCertificate) (semantics : Certificate) : Prop :=
  ∀ (packets : PacketLibrary selector) (rows : RowLibrary selector),
    ProducedRecipe selector packets rows compiler tables semantics

theorem parent (selector : CyclicChoice.Laws) (compiler : Packets.CompilerLaws)
    (tables : TableCertificate) (semantics : Certificate)
    (packetConstruction : PacketConstruction selector) (rowConstruction : RowConstruction selector)
    (sourceAssembly : SourceAssembly selector compiler tables semantics) :
    PCJ38fbfed565f64139_Physical.CachedRecipe selector compiler tables semantics := by
  classical
  let packets : PacketLibrary selector := fun a => Classical.choice (packetConstruction a)
  let rows : RowLibrary selector := fun a printer => Classical.choice (rowConstruction a printer)
  exact producedRecipeProjection selector packets rows compiler tables semantics
    (sourceAssembly packets rows)
end PCJd4d1d9d7d1fa4313_Production
