import Proof.CaseAnalysis.RecoveryGrammarPaddingRow

/-! The original address rows compile their tag, first-field strict bound,
and zero second field, followed by the same reverse conjunction. -/
...
def addressExpr {q bound : ℕ} (row : Fin (bound+1)) (value : Fin 7) (upper : Fin 3) :=
  BoolExpr.all [tagEqualsExpr (n:=q) row value.val,
    firstFieldLessExpr (n:=q) row (rowUpper q row.val upper),secondFieldEqualsExpr (n:=q) row 0]
noncomputable def addressPrefix (upper : Fin 3):=Composition.machine
  (Composition.machine (unary (firstLess upper)) (less (second 0))) (unary foldThree)
noncomputable def addressRow (upper : Fin 3) (next : Selection):=Composition.machine (addressPrefix upper) (fold true next)

theorem address_run {q bound W C D L S B P G : ℕ}
    ...
    (valid : ValidReferences b refs W) (hg : (compileExpr b (addressExpr (q:=q) row value upper)).final.nodes.length≤G)
    ...
    Runs (addressRow upper next) (paddingBudget B) B P source (metadata q bound row.val C B extra)
      (originalState (selectedFields (tag value) q bound row.val C) b graphPre stackPre packet refs)
      (originalState (selectedFields next q bound row.val C) (compileExpr b (addressExpr (q:=q) row value upper)).final
        graphPre stackPre (ZeroPadding.pad B (selectedWord next q bound row.val C))
        (refs++[(compileExpr b (addressExpr (q:=q) row value upper)).output.val])) := by
  ...
  have r1:=less_run room b1 row scalars (firstLess upper) (second 0) 6
  ...
  have forward : Runs (addressPrefix upper) (3*atomBudget B+2) B P source (metadata q bound row.val C B extra)
      (originalState (selectedFields (tag value) q bound row.val C) b graphPre stackPre packet refs)
    ...
    change Runs (addressPrefix upper) (3*atomBudget B+2) B P source (metadata q bound row.val C B extra)
      (originalState (selectedFields (tag value) q bound row.val C) b graphPre stackPre packet refs)
    ...
    rw [rrefs] at prefRun
    exact prefRun
  have whole:=children_run room scalars (addressPrefix upper) (3*atomBudget B+2) (tag value) foldThree next b true xs
