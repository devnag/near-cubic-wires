...
/-! A verbatim framed stream copy keeps both cursors at the next field.
The Pascal row prefix uses it to copy the existing leading one while
positioning the right input at the second original entry. -/
namespace NearCubicWires.RepairOrdinary.CloseoutRowsTouching.FrameStream
...
theorem marker_step (b : Bool) (pre tail out : List Bool):
    step machine (cfg 0 (pre++b::tail) pre.length out)=
      some (cfg (if b then 1 else 2) (pre++b::tail) (pre.length+1) (out++[b])):=by
...
theorem copy_timed (pre bits tail out : List Bool):
    Timed machine (2*bits.length+1)
      ...
      (cfg 2 (pre++frame bits++tail) (pre.length+2*bits.length+1) (out++frame bits)):=by
  induction bits generalizing pre out with
  | nil=>simpa only [frame,List.length_nil,Nat.mul_zero,Nat.add_zero,List.append_assoc,List.cons_append,
      List.nil_append,Bool.false_eq_true,↓reduceIte] using
  ...
  | cons b bits ih=>
    ...
    have hb:step machine (cfg 1 source (pre.length+1) (out++[true]))=
      ...
      simpa [source,frame,List.append_assoc,Nat.add_assoc] using bit_step b (pre++[true]) (frame bits++tail) (out++[true])
    ...
    rw [htime,hpos] at h
    simpa only [source,frame,List.append_assoc,List.cons_append,List.nil_append] using h
...
theorem copy_run (pre bits tail out : List Bool):
    ∃ r,runFrom machine (2*bits.length+1) (cfg 0 (pre++frame bits++tail) pre.length out)=some r ∧
      r.final=cfg 2 (pre++frame bits++tail) (pre.length+2*bits.length+1) (out++frame bits) ∧
      r.steps=2*bits.length+1:=(copy_timed pre bits tail out).run (by rfl)

end NearCubicWires.RepairOrdinary.CloseoutRowsTouching.FrameStream
