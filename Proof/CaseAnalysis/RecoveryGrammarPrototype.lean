...
/-! The fixed grammar prototype is physically printed from five retained
raw scalar tapes. Tape75 is the existing prototype port and73 is its paid log. -/
...
def outputHeads (H : Fin 84→ℕ) (out : List Bool):=Function.update H 75 out.length
def outputData (A : Fin 84→List Bool) (out : List Bool):=Function.update A 75 out
def Appends {s : ℕ} (p : Machine 84 s) (fuel : ℕ) (H : Fin 84→ℕ)
    (A : Fin 84→List Bool) (out result : List Bool) : Prop:=
...
theorem Appends.join {s t u v : ℕ} {p : Machine 84 s} {q : Machine 84 t}
    {H : Fin 84→ℕ} {A : Fin 84→List Bool} {out mid result : List Bool}
...
def slots (source : Fin 84) : Fin 3→Fin 84:=![source,75,73]
noncomputable def scalar (source : Fin 84):=RecoveryFocus.machine (slots source) RecoveryBoundedGrammarFrame.machine

theorem scalar_run (source : Fin 84) (n B : ℕ) (out : List Bool)
    (H : Fin 84→ℕ) (A : Fin 84→List Bool) (hs : source≠75) (hl : source≠73)
...
def wordSlots : Fin 1→Fin 84:=fun _=>75
noncomputable def write (bits : List Bool):=RecoveryFocus.machine wordSlots (HierarchyFixedWord.raw bits)

theorem write_run (bits out : List Bool) (H : Fin 84→ℕ) (A : Fin 84→List Bool) :
...
noncomputable def scalarField (source : Fin 84) (sentinel : Bool):=
  Composition.machine (write (header sentinel)) (scalar source)
def fieldBudget (sentinel : Bool) (n : ℕ):=(header sentinel).length+1+(4*n+8)

theorem field_run (source : Fin 84) (sentinel : Bool) (n B : ℕ) (out : List Bool)
    (H : Fin 84→ℕ) (A : Fin 84→List Bool) (hs : source≠75) (hl : source≠73)
    (hHs : H source=0) (hHl : H 73=0) (hAs : A source=List.replicate n true)
    (hAl : A 73=List.replicate B false) (hB : 2*n+4≤B) :
