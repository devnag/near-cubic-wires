...
/-! The original padding row is exactly tag six, zero first field, zero
second field, and its terminal-true reverse conjunction. -/
...
noncomputable def paddingPrefix:=Composition.machine
  (Composition.machine (unary (first 0)) (unary (second 0))) (unary foldThree)
noncomputable def padding (next : Selection):=Composition.machine paddingPrefix (fold true next)
def paddingBudget (B : ℕ):=4*atomBudget B+3

theorem padding_run {q bound W C D L S B P G : ℕ}
    ...
    (valid : ValidReferences b refs W) (hg : (compileExpr b (paddingRowExpr (n:=q) row)).final.nodes.length≤G)
    ...
    Runs (padding next) (paddingBudget B) B P source (metadata q bound row.val C B extra)
      (originalState (selectedFields (tag 6) q bound row.val C) b graphPre stackPre packet refs)
      (originalState (selectedFields next q bound row.val C) (compileExpr b (paddingRowExpr (n:=q) row)).final
        graphPre stackPre (ZeroPadding.pad B (selectedWord next q bound row.val C))
        (refs++[(compileExpr b (paddingRowExpr (n:=q) row)).output.val])) := by
  let e1:=tagEqualsExpr (n:=q) row 6
  let e2:=firstFieldEqualsExpr (n:=q) row 0
  ...
  have r0:=unary_run room b row scalars (tag 6) (first 0) 0
  ...
  have r1:=unary_run room b1 row scalars (first 0) (second 0) 6
    (graphWord graphPre b1) (saved stackPre refs1) (ZeroPadding.pad B (selectedWord (first 0) q bound row.val C)) source extra
    ...
    (by change b1.nodes.length+3*boundedCircuitFieldLimit q bound≤W;have hh:=alloc.field;omega)
    (h2.trans alloc.graph) (graphSupport _ h1) (v1.support stackPre h1 alloc.graph stackSupport) hs
    (room.pad_packet_length scalars (first 0))
  ...
  have forward : Runs paddingPrefix (3*atomBudget B+2) B P source (metadata q bound row.val C B extra)
      (originalState (selectedFields (tag 6) q bound row.val C) b graphPre stackPre packet refs)
    ...
    change Runs paddingPrefix (3*atomBudget B+2) B P source (metadata q bound row.val C B extra)
      (originalState (selectedFields (tag 6) q bound row.val C) b graphPre stackPre packet refs)
    ...
    rw [rrefs] at prefRun
    exact prefRun
  have whole:=children_run room scalars paddingPrefix (3*atomBudget B+2) (tag 6) foldThree next b true xs
