...
namespace NearCubicWires.PacketsGlue.PrimeCount
...
7–10 next divisor, 12 count, 13–19 next candidate, 20 halt. -/
def machine : Machine 4 21 where
  descriptionBits := 0
  start := 0
  ...
  rule := fun q b =>
    ...
    else if q.val = 12 then some ⟨13, ![none, some true, none, none], ![.stay, .right, .stay, .stay]⟩
...
def cfg (q : Fin 21) (n h0 m : ℕ) (C : List Bool) (h2 : ℕ) (D : List Bool) (h3 : ℕ) : Configuration 4 21 :=
...
theorem s12 : step machine (cfg 12 n h0 m C h2 D h3) = some (cfg 13 n h0 (m+1) C h2 D h3) := by
  simp [step, machine, cfg]
  apply configuration_ext
  · rfl
  · funext i; fin_cases i <;> simp [applyAction, HeadMove.apply]
...
theorem timed_congr {t t' : ℕ} {c c' d d' : Configuration 4 21} (h : Timed machine t c d)
    (ht : t = t') (hc : c = c') (hd : d = d') : Timed machine t' c' d' := by
  subst ht hc hd
  exact h

theorem tr {t u : ℕ} {c d d' e : Configuration 4 21} (h1 : Timed machine t c d) (h2 : Timed machine u d' e)
    (h : d = d') : Timed machine (t + u) c e := by
  subst h
  exact h1.trans h2

theorem one {c d : Configuration 4 21} (hq : c.control.val ≠ 20) (h : step machine c = some d) :
    Timed machine 1 c d :=
  Timed.single (by simp [machine, hq]) h

/-- A family of configurations stepping into each other is a timed walk. -/
theorem walk (f : ℕ → Configuration 4 21) (a : ℕ) : ∀ s : ℕ,
...
theorem scan_tt (q : Fin 21) (hq : q = 4 ∨ q = 5) (c d k i i' s : ℕ) (hi : i + s ≤ c) (hi' : i' + s ≤ d) :
...
def pc : ℕ → ℕ
  | 0 => 0
  | c + 1 => pc c + if (c + 1).Prime then 1 else 0

theorem pc_eq (x : ℕ) : pc x = (SupplierPrime.primesUpTo x).card := by
  induction x with
  | zero => simp [pc, SupplierPrime.primesUpTo, Finset.range_one, Finset.filter_singleton, Nat.not_prime_zero]
  | succ c ih =>
...
def outerCfg (j m k : ℕ) : Configuration 4 21 := cfg 3 n j m (sent (j + 1) 0) 1 (sent 2 k) 1

theorem outer_one (j k : ℕ) (hj : 1 ≤ j) (hjn : j < n) : ∃ T k', T ≤ 6 * (n + 3) ^ 2 ∧
    Timed machine T (outerCfg n j (pc j) k) (outerCfg n (j + 1) (pc (j + 1)) k') := by
  have hr := read_rep n j
  have w0 := one (c := outerCfg n j (pc j) k) (by simp [outerCfg, cfg])
    (s3t n j (pc j) _ _ _ _ (by rw [hr]; simp [hjn]))
  ...
  obtain ⟨T, k', hT, h⟩ := inner n j (pc j) (j + 1) hc2 (Nat.minFac (j + 1) - 2) 2 k (le_refl 2) (by omega)
  ...
  · have w2 := one (c := cfg 12 n j (pc j) (sent (j + 1) 0) (1 + (j + 1)) (sent (Nat.minFac (j + 1)) k')
      (1 + Nat.minFac (j + 1))) (by simp [cfg]) (s12 n j (pc j) _ _ _ _)
    have w3 := post n j (pc j + 1) (j + 1) (Nat.minFac (j + 1)) k'
    ...
    simp only [outerCfg, pc, if_pos hp]
  · have w3 := post n j (pc j) (j + 1) (Nat.minFac (j + 1)) k'
    refine ⟨1 + T + (j + 1 + Nat.minFac (j + 1) + 7), k' + Nat.minFac (j + 1) - 2, by omega, ?_⟩
    have t := tr (w0.trans w1) w3 rfl
    refine timed_congr t rfl rfl ?_
    simp only [outerCfg, pc, if_neg hp, Nat.add_zero]

theorem outer_loop : ∀ s j k, 1 ≤ j → j + s = n → ∃ T k', T ≤ s * (6 * (n + 3) ^ 2) ∧
    Timed machine T (outerCfg n j (pc j) k) (outerCfg n n (pc n) k') := by
  intro s
  induction s with
  | zero =>
    intro j k _ hjs
...
theorem pc_one : pc 1 = 0 := by simp [pc, Nat.not_prime_one]

/-- **The prime counter**: from `1^n` it writes `1^(π n)` on tape 1 and halts. -/
theorem run (n : ℕ) : ∃ T H A, Step machine T (fun _ => 0) ![List.replicate n true, [], [], []] H A ∧
    A 1 = List.replicate (pc n) true ∧ T ≤ 6 * (n + 3) ^ 3 + 4 := by
  have w0 := init n
  obtain ⟨j, T1, k, hj, hT1, hpc, w1⟩ : ∃ j T1 k, n ≤ j ∧ T1 ≤ n * (6 * (n + 3) ^ 2) ∧ pc j = pc n ∧
      Timed machine T1 (outerCfg n 1 0 0) (outerCfg n j (pc j) k) := by
    by_cases hn : 1 ≤ n
    · obtain ⟨T, k', hT, w⟩ := outer_loop n (n - 1) 1 0 (le_refl 1) (by omega)
      ...
      · have := Nat.mul_le_mul_right (6 * (n + 3) ^ 2) (show n - 1 ≤ n by omega)
        omega
      · simpa [pc_one] using w
    · refine ⟨1, 0, 0, by omega, by simp, ?_, ?_⟩
      · have : n = 0 := by omega
        subst this; rfl
      · simpa [pc_one] using Timed.refl machine (outerCfg n 1 0 0)
  have hr := read_rep n j
  have w2 := one (c := outerCfg n j (pc j) k) (by simp [outerCfg, cfg])
    (s3f n j (pc j) _ _ _ _ (by rw [hr]; simp; omega))
  have t := (w0.trans w1).trans w2
  obtain ⟨r, hr', hf, hs⟩ := t.run (by simp [machine, cfg])
  refine ⟨3 + T1 + 1, _, _, ⟨r, ?_, rfl, rfl, by omega⟩, ?_, ?_⟩
  · have hc : (⟨machine.start, fun _ => 0, ![List.replicate n true, [], [], []]⟩ : Configuration 4 21) =
        cfg 0 n 0 0 [] 0 [] 0 := by
      apply configuration_ext
      · rfl
...
end NearCubicWires.PacketsGlue.PrimeCount
