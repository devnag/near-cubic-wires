import Proof.Packets.ScalarTwice
...
namespace PCJ9eff70d512234a4c_Fixed.Materializer.ScalarTwiceInto
open NearCubicWires NearCubicWires.LocalBitMultitape NearCubicWires.RepairOrdinary
open NearCubicWires.ExtDecompositionBatch NearCubicWires.RepairSource.VerifierDecoding
noncomputable section

def slots {t : Nat} (scalar log counter : Fin t) : Fin 3→Fin t := ![scalar,log,counter]
def machine {t : Nat} (scalar log counter : Fin t) := RecoveryFocus.machine (slots scalar log counter) ScalarTwice.machine
...
theorem run {t : Nat} (u old R N : Nat) (scalar log counter : Fin t)
    ...
    (hn : 2*N<2^u) (hr : 2*u+1≤R) :
    Step (machine scalar log counter) (ScalarTwice.budget u N) H A H
      (Function.update A scalar (ZeroPadding.pad R (frame (SignedSortKey.binary u (2*N))))) := by
  ...
  apply PhysicalFocusBoundary.focus (ScalarTwice.padded_run u old R N hn hr)
  ...
  · intro i;fin_cases i <;> first | exact hs.symm | exact hl.symm | exact hc.symm
  · intro i;fin_cases i
    · change ZeroPadding.pad R (frame (SignedSortKey.binary u (2*N)))=
        Function.update A scalar (ZeroPadding.pad R (frame (SignedSortKey.binary u (2*N)))) scalar
      simp only [Function.update_self]
    · change ZeroPadding.pad 0 (List.replicate R false)=
        Function.update A scalar (ZeroPadding.pad R (frame (SignedSortKey.binary u (2*N)))) log
      simpa only [ZeroPadding.pad_zero,Function.update_of_ne hsl] using al.symm
    · change ZeroPadding.pad R (CompareMachine.word N)=
        Function.update A scalar (ZeroPadding.pad R (frame (SignedSortKey.binary u (2*N)))) counter
...
theorem zero_run {t : Nat} (u old R N : Nat) (scalar log counter : Fin t)
    ...
    (al : A log=List.replicate R false) (ac : A counter=ZeroPadding.pad R (CompareMachine.word N))
    (hn : 2*N<2^u) (hr : 2*u+1≤R) :
    Step (zeroMachine scalar log counter) (ScalarTwice.budget u N+4) H A H
      (Function.update A scalar (ZeroPadding.pad R (frame (SignedSortKey.binary u (2*N))))) := by
  ...
  have last:=PhysicalIndexReload.move_run counter .left (Function.update H counter 1)
    (Function.update A scalar (ZeroPadding.pad R (frame (SignedSortKey.binary u (2*N)))))
  ...
  have hf : 1+1+(ScalarTwice.budget u N+1+1)=ScalarTwice.budget u N+4 := by omega
  rw [hf] at h
  exact h

end
end PCJ9eff70d512234a4c_Fixed.Materializer.ScalarTwiceInto
