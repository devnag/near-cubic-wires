import Proof.Packets.DenseAtomProgram
...
bytes are the exact states of DenseAtomProgram. -/
...
def H (tag : Nat) (cs : List Pair) (j : Nat) := NativeIndexedAtom.heads 0 (codePrefix tag cs j).length
def A (C R tag : Nat) (cs : List Pair) (initial : List Packet) (j : Nat) :=
  NativeIndexedAtom.data C R (cacheWord cs) (codes tag cs)
    (PacketVector.bank R (table C tag cs initial j)) (index cs j)
def bodyBudget (C R : Nat) := 128*(C+1)*(R+1)

theorem iteration_run (C R tag : Nat) (cs : List Pair) (initial : List Packet)
    ...
    Step NativeIndexedAtom.machine (bodyBudget C R) (H tag cs j) (A C R tag cs initial j)
      (H tag cs (j+1)) (A C R tag cs initial (j+1)) := by
  let i:=index cs j
  let c:=code tag cs j
  let p:=atom cs j
  let ps:=table C tag cs initial j
  have hi : i<cs.length := index_lt cs j hj
  have hp : p∈cs := atom_mem cs j hj
  have hc : c<C := hcodes i hi
  have hpslen : ps.length=C := (table_length C tag cs initial j).trans hinit
  ...
  have hps : ∀P∈ps,PacketVector.Fits R P := table_fits C R tag cs initial hinits hanswer j (by omega)
  ...
  have codeSource : codePrefix tag cs j++NativeLiteralCode.word c++suffix tag cs j=codes tag cs := code_split tag cs j hj
  ...
  have actual:=NativeIndexedAtom.run C R c (cs.take i) (cacheWord (cs.drop (i+1)))
    (codePrefix tag cs j) (suffix tag cs j) (PacketVector.bank R (ps.take c))
  ...
  have nextCursor : (codePrefix tag cs j).length+c+6=(codePrefix tag cs (j+1)).length :=
    (prefix_length_succ tag cs j hj).symm
