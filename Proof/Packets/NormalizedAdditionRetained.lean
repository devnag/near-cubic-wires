...
/-! One actual fixed addition pipeline: frame the left bank in reverse, then the right bank,
...
theorem run_retained (B : Nat) (left right : List (List Bool))
  ...
  obtain ⟨b,hb,hbs,bout,bhead,bcount,bcountHead,bWidth⟩ :=
    NormalizeCold.run_width B (left.reverse++right) (sum_width B left right hl hr)
...
theorem run_both (B : Nat) (left right : List (List Bool))
    (hl : ∀ bits∈left,bits.length=B) (hr : ∀ bits∈right,bits.length=B) :
    ∃ r,runFrom machine (budget B left right) (entry B left right)=some r ∧
      r.steps≤budget B left right ∧
      r.final.tapes 20=(NormalizerOrder.ordered (left.reverse++right)).flatten ∧
      ...
      r.final.tapes 24=UnaryTemplate.tape B ∧
      r.final.tapes 25=left.flatten ∧
      r.final.tapes 28=CompareMachine.word left.length ∧
      r.final.tapes 26=right.flatten ∧
      r.final.tapes 27=CompareMachine.word right.length := by
  obtain ⟨a,ha,ah,adata,has⟩ := addition_step B left right hl hr
  obtain ⟨b,hb,hbs,bout,bhead,bcount,bcountHead,bWidth⟩ :=
    NormalizeCold.run_width B (left.reverse++right) (sum_width B left right hl hr)
  have h := TapeEmbedding.run_embed NormalizeCold.machine extraHeads (extras B left right) _ _ b hb
  ...
  rw [←join] at h
  have result := Composition.run_join additionMachine normalizeMachine _ _ _ a _ ha h
  refine ⟨_,result,?_,?_,?_,?_,?_,?_,?_,?_,?_,?_,?_⟩
  · change a.steps+1+b.steps≤budget B left right
    unfold budget
    omega
  · change (TapeEmbedding.receipt extraHeads (extras B left right) b).final.tapes 20=_
  ...
  · change (TapeEmbedding.receipt extraHeads (extras B left right) b).final.tapes 26=_
    simp [TapeEmbedding.receipt,TapeEmbedding.config,Fin.addCases,extras]
  · change (TapeEmbedding.receipt extraHeads (extras B left right) b).final.tapes 27=_
    simp [TapeEmbedding.receipt,TapeEmbedding.config,Fin.addCases,extras]
