...
namespace RowsConstruction.I2c.Measure
...
open NearCubicWires.RepairRepresentation NearCubicWires.RepairSource.ProjectionNormalization
...
theorem flat_length_ge {q : Nat} : ∀ gs : List (ExactThresholdGate q), gs.length ≤ (gs.flatMap exactWord).length
  | [] => by simp
  | g :: gs => by
    have ih := flat_length_ge gs
    have h1 : 1 ≤ (exactWord g).length := by
      simp only [exactWord, intWord, List.length_append, List.length_cons]
      omega
    simp only [List.flatMap_cons, List.length_append, List.length_cons]
    omega

theorem budget_le {q : Nat} (gs : List (ExactThresholdGate q)) :
    budget gs ≤ 256*((exactListWord gs).length+q+1)^2 := by
  have hn : natBitLength gs.length ≤ gs.length+1 := Nat.add_le_add_right (Nat.log_le_self 2 gs.length) 1
  have hw : (exactListWord gs).length = (natWord gs.length).length+(gs.flatMap exactWord).length := by
    simp [exactListWord]
  have hnat := DecompositionSource.natWord_length gs.length
  have hlen := flat_length_ge gs
  have ht : gs.take gs.length = gs := List.take_length
  unfold budget rawBudget DecompositionSource.Count.budget DecompositionCachedChild.budget PCPPQueryNatural.budget
    MatrixDimensionPrepare.budget
  rw [ht]
  generalize (exactListWord gs).length = X at hw ⊢
  generalize (gs.flatMap exactWord).length = F at hw hlen ⊢
  generalize gs.length = n at hn hw hlen hnat ⊢
  generalize natBitLength n = b at hn hnat ⊢
  nlinarith
end RowsConstruction.I2c.Measure
