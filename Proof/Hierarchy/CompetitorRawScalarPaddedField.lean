import Proof.Hierarchy.CompetitorRawScalarPaddedAppend
...
namespace NearCubicWires.RepairOrdinary.CompetitorRawScalarPaddedField
...
noncomputable def program {t : ℕ} (source target counter : Fin t) :=
  RecoveryFocus.machine (slots source target counter) CompetitorRawScalarEmit.machine
...
theorem focused_heads {t : ℕ} (source target counter : Fin t)
    ...
    (RecoveryFocus.config (slots source target counter) heads ambient
      (CompetitorRawScalarEmit.cfg q bits out cap)).heads=Function.update heads target out.length := by
...
theorem field_run {t : ℕ} (source target counter : Fin t)
    ...
    (hsource : ambient source=ZeroPadding.pad cap (frame bits))
    ...
    ∃ r : ExecutionReceipt t 4,
      ...
      r.final.heads=Function.update heads target (out++bits).length ∧
      r.final.tapes=Function.update ambient target (out++bits) ∧ r.steps=4*bits.length+3 := by
  have hi := slots_injective source target counter hst hsc htc
  obtain ⟨base,hr,hf,hs'⟩ := CompetitorRawScalarEmit.padded_append_run bits out cap hcap
  obtain ⟨r,hrun,hfinal,hsteps⟩ := RecoveryFocus.run_config (slots source target counter) hi
    CompetitorRawScalarEmit.machine heads ambient _ _ base hr
  have hin : RecoveryFocus.config (slots source target counter) heads ambient (CompetitorRawScalarEmit.cfg 0 bits out cap)=
  ...
  · rw [hfinal,hf]
    change install (slots source target counter) ambient
      (CompetitorRawScalarEmit.cfg 3 bits (out++bits) cap).tapes=Function.update ambient target (out++bits)
    funext i
    by_cases hit : i=target
    · subst i
...
end NearCubicWires.RepairOrdinary.CompetitorRawScalarPaddedField
