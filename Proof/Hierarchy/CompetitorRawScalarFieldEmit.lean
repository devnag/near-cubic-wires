import Proof.Hierarchy.CompetitorResidueCount
...
namespace NearCubicWires.RepairOrdinary.CompetitorRawScalarFieldEmit
...
theorem focused_heads {t : ℕ} (source target counter : Fin t)
    ...
    (RecoveryFocus.config (slots source target counter) heads ambient
      (CompetitorRawScalarAppend.cfg q bits out cap)).heads=Function.update heads target out.length := by
...
theorem field_run {t : ℕ} (source target counter : Fin t)
  ...
  have hi := slots_injective source target counter hst hsc htc
  obtain ⟨base,hr,hf,hs'⟩ := CompetitorRawScalarAppend.raw_append_run bits out cap hcap
  ...
  have hin : RecoveryFocus.config (slots source target counter) heads ambient (CompetitorRawScalarAppend.cfg 0 bits out cap)=
  ...
  · rw [hfinal,hf]
    change install (slots source target counter) ambient
      (CompetitorRawScalarAppend.cfg 3 bits (out++bits) cap).tapes=Function.update ambient target (out++bits)
    funext i
    by_cases hit : i=target
    · subst i
...
end NearCubicWires.RepairOrdinary.CompetitorRawScalarFieldEmit
