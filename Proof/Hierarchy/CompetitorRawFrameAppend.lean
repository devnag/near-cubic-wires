import Proof.Hierarchy.CompetitorThresholdTail

/-! Framed appending from an exact scalar frame with a retained paid zero
counter. The source head is restored and the output append head advances. -/
namespace NearCubicWires.RepairOrdinary.CompetitorRawFrameAppend
open LocalBitMultitape RecoveryExecution
open CompetitorReusableDecision CompetitorFrameAppend
...
theorem raw_append_run (bits out : List Bool) (cap : ℕ) (hc : 2*bits.length+1≤cap) :
    ∃ r : ExecutionReceipt 3 4,
      ...
      r.final=cfg 3 bits (out++frame bits) cap ∧ r.steps=4*bits.length+3 := by
...
end NearCubicWires.RepairOrdinary.CompetitorRawFrameAppend
