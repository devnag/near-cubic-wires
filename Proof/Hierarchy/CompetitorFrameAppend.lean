import Proof.Hierarchy.CompetitorSumEntry

/-! Append one actual framed scalar to a retained output stream. The
...
namespace NearCubicWires.RepairOrdinary.CompetitorFrameAppend
...
theorem marker_step (pre tail out : List Bool) :
    step machine (scan 0 (pre++true::tail) pre.length out)=
      some (scan 1 (pre++true::tail) (pre.length+1) (out++[true])) := by
...
theorem delimiter_step (pre tail out : List Bool) :
    step machine (scan 0 (pre++false::tail) pre.length out)=
      some (reset 2 (pre++false::tail) (out++[false]) (pre.length+1) 0) := by
  ...
  · funext i
    fin_cases i <;> simp [applyAction,action,Streaming.write_append,reset,List.replicate_add]
...
theorem scan_timed (pre bits suffix out : List Bool) :
    Timed machine (2*bits.length+1) (scan 0 (pre++frame bits++suffix) pre.length out)
      (reset 2 (pre++frame bits++suffix) (out++frame bits) (pre.length+2*bits.length+1) 0) := by
  ...
  | cons bit bits ih =>
    ...
    have hfirst : Timed machine 1 (scan 0 source pre.length out)
        (scan 1 source (pre.length+1) (out++[true])) := by
      simpa [source,frame,List.append_assoc] using Timed.single (by rfl)
        (marker_step pre (bit::frame bits++suffix) out)
    have hsecond : Timed machine 1 (scan 1 source (pre.length+1) (out++[true]))
        (scan 0 source (pre.length+2) (out++[true,bit])) := by
      simpa [source,frame,List.append_assoc,Nat.add_assoc] using Timed.single (by rfl)
        (bit_step (pre++[true]) (frame bits++suffix) (out++[true]) bit)
    have htail : Timed machine (2*bits.length+1) (scan 0 source (pre.length+2) (out++[true,bit]))
        (reset 2 source (out++frame (bit::bits)) (pre.length+2*(bit::bits).length+1) 0) := by
...
theorem append_run (bits suffix out : List Bool) :
    ∃ r : ExecutionReceipt 3 4,
      ...
      r.final=reset 3 (frame bits++suffix) (out++frame bits) 0 (2*bits.length+1) ∧
  ...
  have hreset := reset_timed (frame bits++suffix) (out++frame bits) (2*bits.length+1) 0
...
end NearCubicWires.RepairOrdinary.CompetitorFrameAppend
