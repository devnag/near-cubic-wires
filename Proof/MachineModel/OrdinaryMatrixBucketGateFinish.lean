import Proof.MachineModel.OrdinaryMatrixBucketGateBootstrap
...
namespace NearCubicWires.RepairOrdinary.MatrixBucketGateFinish
open LocalBitMultitape RecoveryExecution MatrixScoreBatch MatrixBucketGateNativeLoop
...
noncomputable def before (r : Request) (unused : Fin 3 → List Bool) (s : MatrixBucketGateLoop.Store r) :=
  MatrixBucketGateNativeLoop.cfg r 3 r.Gates (MatrixBatchGateNativeLoop.output r)
    (MatrixBatchGateNativeLoop.output r).length (MatrixBucketGateNativeLoop.output r) unused s
noncomputable def final (r : Request) (unused : Fin 3 → List Bool) (s : MatrixBucketGateLoop.Store r) : Configuration 36 2 :=
  ⟨1,Function.update (before r unused s).heads 8 ((MatrixBucketGateNativeLoop.output r).length+1),
...
theorem finish_run (r : Request) (unused : Fin 3 → List Bool) (s : MatrixBucketGateLoop.Store r) :
  ...
  have hhead : (before r unused s).heads 8=(MatrixBucketGateNativeLoop.output r).length := by
    simp only [before,MatrixBucketGateNativeLoop.cfg_heads]
    rfl
  have htape : (before r unused s).tapes 8=ZeroPadding.pad (MatrixScoreReusableRanks.D r) (MatrixBucketGateNativeLoop.output r) := by
    simp only [before,MatrixBucketGateNativeLoop.cfg_tapes]
    rfl
...
end NearCubicWires.RepairOrdinary.MatrixBucketGateFinish
