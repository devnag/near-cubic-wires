import Proof.Amplification.RecoveryOuterTableLoop

/-! The outer table physical Boolean return is proved over an abstract source
...
def writeResult (bit : Bool) : Machine 85 2 where
  descriptionBits := 0
  start := 0
  halted := fun q=>q.val==1
  rule := fun q _=>if q.val=0 then some ⟨1,fun i=>if i=50 then some bit else none,fun _=>.stay⟩ else none

theorem write_result_run (heads : Fin 85→Nat) (tapes : Fin 85→List Bool) (old bit : Bool)
    (hh : heads 50=0) (ht : tapes 50=[old]) :
    ∃ r,runFrom (writeResult bit) 1 (⟨0,heads,tapes⟩ : Configuration 85 2)=some r ∧
      r.final=⟨1,heads,Function.update tapes 50 [bit]⟩ ∧ r.steps=1 := by
  have h : step (writeResult bit) (⟨0,heads,tapes⟩ : Configuration 85 2)=
      some (⟨1,heads,Function.update tapes 50 [bit]⟩ : Configuration 85 2) := by
    apply congrArg some
...
def programs {s : Nat} (p : Machine 85 s) : (j : Fin 3)→Machine 85 (sizes s j)
  | ⟨0,_⟩=>p
  | ⟨1,_⟩=>writeResult false
  | ⟨2,_⟩=>writeResult true
  | ⟨n+3,h⟩=>False.elim (by omega)
def next {s : Nat} (result : Fin s→Bool) : (j : Fin 3)→Fin (sizes s j)→(Fin 85→Bool)→Option (Fin 3)
  | ⟨0,_⟩,q,_=>if result q then some 2 else some 1
  | ⟨1,_⟩,_,_=>none
  | ⟨2,_⟩,_,_=>none
  | ⟨n+3,h⟩,_,_=>False.elim (by omega)
noncomputable def machine {s : Nat} (p : Machine 85 s) (result : Fin s→Bool) :=
  RecoveryCalls.machine (sizes s) (programs p) 0 (next result)

theorem return_trace {s : Nat} (p : Machine 85 s) (result : Fin s→Bool) (fuel : Nat)
    (source : Configuration 85 s) (first : ExecutionReceipt 85 s)
...
theorem return_run {s : Nat} (p : Machine 85 s) (result : Fin s→Bool) (fuel : Nat)
    (source : Configuration 85 s) (first : ExecutionReceipt 85 s)
    (hr : runFrom p fuel source=some first) (old : Bool)
    (hh : first.final.heads 50=0) (ht : first.final.tapes 50=[old]) :
