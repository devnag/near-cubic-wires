import Proof.Amplification.RecoveryTseitinPrepareLayout
...
theorem erase_range (j : Fin 236) : 3 ≤ (eraseSlots j).val := by
  refine Fin.addCases (m := 235) (n := 1) (fun a=>?_) (fun a=>?_) j
...
theorem clear_run (cap log : Nat) (ambient : Fin 239→List Bool)
  ...
  let result : Fin 236→List Bool := Fin.addCases (m := 235) (n := 1)
    (motive := fun _=>List Bool)
    (Fin.addCases (m := 234) (n := 1) (motive := fun _=>List Bool)
