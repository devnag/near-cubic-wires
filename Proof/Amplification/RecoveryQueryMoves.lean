import Proof.Amplification.RecoveryQueryClear
...
theorem literal_run (destination : Fin 357) (word : List Bool) (cap : Nat)
    (ambient : Fin 357→List Bool) (hd : destination≠356) (hc : word.length ≤ cap)
...
theorem copy_run (k : Fin 3) (cap : Nat) (bits padding : List Bool) (ambient : Fin 357→List Bool)
    (hc : (frame bits).length ≤ cap) (hs : ambient (sourceSlot k)=frame bits++padding)
    (hb : ambient (destinationSlot k)=List.replicate cap false)
    (hl : ambient 356=List.replicate cap false) :
    ClockJoin.ReadyRun (copyMachine k) (4*bits.length+4) ambient
      (Function.update ambient (destinationSlot k) (ZeroPadding.pad cap (frame bits))) := by
  classical
  have hc' : 2*bits.length+1 ≤ cap := by simpa using hc
  have h := (PCPFieldMoves.ready_run bits padding cap cap).focus (copySlots k) (copy_injective k) ambient (by
    intro j
    fin_cases j <;> simpa [copySlots] using (by assumption))
  have hout : PCPFieldMoves.output [] bits padding cap cap=
      ![ambient (sourceSlot k),ZeroPadding.pad cap (frame bits),ambient 356] := by
    funext j
    fin_cases j <;> simp [PCPFieldMoves.output,hs,hl,max_eq_left hc']
  rw [hout] at h
  have he : install (copySlots k) ambient
      ![ambient (sourceSlot k),ZeroPadding.pad cap (frame bits),ambient 356]=
      Function.update ambient (destinationSlot k) (ZeroPadding.pad cap (frame bits)) := by
    have hi := install_first (copySlots k) (copy_injective k) ambient (ZeroPadding.pad cap (frame bits)) (ambient 356)
    change install (copySlots k) ambient
        ![ambient (sourceSlot k),ZeroPadding.pad cap (frame bits),ambient 356]=
      Function.update (Function.update ambient (destinationSlot k) (ZeroPadding.pad cap (frame bits))) 356 (ambient 356) at hi
    rw [hi]
    apply Function.update_eq_self_iff.mpr
    have hne : (356 : Fin 357)≠destinationSlot k := by fin_cases k <;> decide
