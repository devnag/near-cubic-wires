import Proof.Amplification.RecoveryCaseOneEvaluationPayload

/-! Physically frame the generated table and original address for the
existing ordinary complete-table evaluator, including its return scan. -/
...
def framedBudget (left right : List Bool) := 2*budget left right+4*(left++frame right).length+7
...
theorem framed_ready (left right : List Bool) : ∃ out,
    ClockJoin.ReadyRun framedMachine (framedBudget left right) (fields left right) out ∧
      out 6=frame (left++frame right) := by
  ...
  have hbudget : 2*base.steps+4*(left++frame right).length+7≤framedBudget left right := by
    unfold framedBudget
    omega
  have hm:=run_moreFuel framedMachine _
    (framedBudget left right-(2*base.steps+4*(left++frame right).length+7)) _ r hr
