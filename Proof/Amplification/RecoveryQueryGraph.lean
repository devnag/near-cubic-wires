import Proof.Amplification.RecoveryQueryOperands
...
noncomputable def atCall (j : Fin 9) (tapes : Fin 357→List Bool) :=
  controlConfig (RecoveryCalls.code sizes j) (initialConfiguration (programs j) tapes)
def Path (j k : Fin 9) (fuel : Nat) (before after : Fin 357→List Bool) : Prop :=
  ∃ n ≤ fuel,Timed machine n (atCall j before) (atCall k after)

theorem Path.refl (j : Fin 9) (tapes : Fin 357→List Bool) : Path j j 0 tapes tapes :=
  ⟨0,Nat.le_refl _,Timed.refl _ _⟩

theorem Path.trans {j k l : Fin 9} {a b : Nat} {x y z : Fin 357→List Bool}
    (h : Path j k a x y) (g : Path k l b y z) : Path j l (a+b) x z := by
  obtain ⟨n,hn,ht⟩ := h
  obtain ⟨m,hm,gt⟩ := g
  exact ⟨n+m,by omega,ht.trans gt⟩

theorem Path.enlarge {j k : Fin 9} {a b : Nat} {x y : Fin 357→List Bool}
    (h : Path j k a x y) (hab : a ≤ b) : Path j k b x y := by
  obtain ⟨n,hn,ht⟩ := h
  exact ⟨n,hn.trans hab,ht⟩

theorem ready_path (j k : Fin 9) (fuel : Nat) (input output : Fin 357→List Bool)
...
theorem finish_path {fuel last : Nat} {input middle output : Fin 357→List Bool}
    (h : Path 0 8 fuel input middle)
    (g : ClockJoin.ReadyRun (programs 8) last middle output) :
    ClockJoin.ReadyRun machine (fuel+last+1) input output := by
  obtain ⟨n,hn,hpath⟩ := h
  obtain ⟨r,hr,ht,hh,hs⟩ := g
  obtain ⟨m,hm,hlast⟩ := stop_receipt sizes programs 0 next 8 last
    (initialConfiguration (programs 8) middle) r hr (by rfl)
  have he : RecoveryCalls.stopped sizes r.final.heads r.final.tapes=
      RecoveryCalls.stopped sizes (fun _ : Fin 357=>0) output := by
    apply configuration_ext
    · rfl
    · exact funext hh
    · exact ht
