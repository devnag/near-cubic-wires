...
/-! The actual Case-1 input is its original framed dimension followed by
the complete recovered proof table, physically unframed and appended. -/
...
noncomputable def last := RecoveryFocus.machine rightSlots GeneratedAmplifier.Copy.machine
noncomputable def machine := Composition.machine first last
def input (left right : List Bool) : Fin 4→List Bool := ![frame left,frame right,[],[]]
def budget (left right : List Bool) := (4*left.length+3)+1+(2*right.length+1)

theorem pair_run (left right : List Bool) : ∃ r,
    run machine (budget left right) (input left right)=some r ∧
      r.final.tapes 2=frame left++right ∧
      r.final.heads 2=(frame left++right).length ∧ r.steps≤budget left right := by
  ...
  obtain ⟨a,ha,_ac,aSteps,ah,aT,ak⟩ := RecoveryFocus.dock leftSlots left_injective
    CompetitorFrameAppend.machine _ (fun _ : Fin 4=>0) (input left right)
  ...
  have hhead : a.final.heads 2=(frame left).length := by
    ...
    rw [lf] at h
    simpa only [leftSlots,CompetitorFrameAppend.reset,Matrix.cons_val_one,Matrix.cons_val_zero,List.nil_append] using h
  obtain ⟨r,hr,rf,rSteps⟩ := GeneratedAmplifier.Copy.copy_run [] right [] (frame left)
  have hh : ∀ i : Fin 2,a.final.heads (rightSlots i)=
      (GeneratedAmplifier.Copy.cfg 0 ([]++frame right++[]) 0 (frame left)).heads i := by
    intro i; fin_cases i
    · exact (ak 1 (by decide)).1
    · exact hhead
  have ht : ∀ i : Fin 2,a.final.tapes (rightSlots i)=
      (GeneratedAmplifier.Copy.cfg 0 ([]++frame right++[]) 0 (frame left)).tapes i := by
    intro i; fin_cases i
  ...
  obtain ⟨b,hb,_bc,bSteps,bh,bt,_bk⟩ := RecoveryFocus.dock rightSlots right_injective
    GeneratedAmplifier.Copy.machine _ a.final.heads a.final.tapes _ hh ht r hr
...
theorem copy_forward : CursorRestore.NoLeft GeneratedAmplifier.Copy.machine 1 := by
  intro q bits a ha
  fin_cases q <;> cases hb : bits 0 <;> simp [GeneratedAmplifier.Copy.machine,hb] at ha
...
theorem forward : CursorRestore.NoLeft machine 2 :=
  CursorRestore.composition_forward _ _ _
    ...
    (CursorRestore.focus_forward rightSlots right_injective GeneratedAmplifier.Copy.machine 1
      copy_forward)
