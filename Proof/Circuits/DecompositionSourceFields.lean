...
/-! Paid native signed-field copying. The finite controller consumes the
sign bit and the following literal natWord, retaining both stream cursors. -/
namespace NearCubicWires.RepairOrdinary.DecompositionSource.Fields
open LocalBitMultitape RepairRepresentation PCPPQueryField Streaming RecoveryExecution
...
def sign : Machine 3 2 where
  ...
  rule := fun _ bs => some ⟨1,![none,none,some (bs 0)],![.right,.stay,.right]⟩

def machine := Composition.machine sign (PCPPQueryField.machine true)
def cost (z : ℤ) := 2*natBitLength z.natAbs+5
def saved (z : ℤ) (backing : List Bool) := PCPPQueryField.saved z.natAbs backing

theorem sign_run (pre tail backing out : List Bool) (b : Bool) :
    ∃ r,runFrom sign 1 (store 0 (pre++b::tail) pre.length backing out)=some r ∧
      r.final=store 1 (pre++b::tail) (pre.length+1) backing (out++[b]) ∧ r.steps=1 := by
  have hs : step sign (store 0 (pre++b::tail) pre.length backing out)=
      some (store 1 (pre++b::tail) (pre.length+1) backing (out++[b])) := by
    simp [step,sign,store,Configuration.scanned,read_append]
...
theorem int_run (pre tail backing out : List Bool) (z : ℤ) :
    ∃ r,runFrom machine (cost z)
      ...
      r.final=store 5 (pre++intWord z++tail) (pre.length+(intWord z).length)
        (saved z backing) (out++intWord z) ∧ r.steps=cost z := by
  let b := decide (z<0)
  let source := pre++intWord z++tail
  obtain ⟨first,hfirst,hff,hfs⟩ := sign_run pre (natWord z.natAbs++tail) backing out b
  have hsource : pre++b::(natWord z.natAbs++tail)=source := by simp [source,intWord,b]
  rw [hsource] at hfirst hff
  obtain ⟨last,hl,hlf,hls⟩ := nat_run true (pre++[b]) tail backing (out++[b]) z.natAbs
  have hsource2 : (pre++[b])++natWord z.natAbs++tail=source := by simp [source,intWord,b,List.append_assoc]
  rw [hsource2] at hl hlf
  have hi : cfg 0 source (pre++[b]).length backing 0 (out++[b])=
      Composition.restart first.final (PCPPQueryField.machine true).start := by
    rw [hff]
    simp [cfg,store,Composition.restart,PCPPQueryField.machine]
  rw [hi] at hl
  have hj := Composition.run_join sign (PCPPQueryField.machine true) 1
  ...
  · change Composition.rightConfig 2 last.final=_
    rw [hlf]
    simp only [selected,↓reduceIte,List.length_append,List.length_singleton]
    have hp : pre.length+1+2*natBitLength z.natAbs+1=pre.length+(intWord z).length := by
      rw [intWord,List.length_cons,natWord_length]; omega
    have ho : (out++[b])++natWord z.natAbs=out++intWord z := by simp [intWord,b,List.append_assoc]
    rw [hp,ho]
    rfl
...
noncomputable def loop := RepairSource.VerifierDecoding.RepeatMachine.machine machine (fun _ _ => true)
...
theorem remaining (pre tail backing out : List Bool) (zs : List ℤ) (total pos : ℕ)
    ...
    Timed loop ((zs.flatMap intWord).length+5*zs.length+total+3)
      ...
      (cfg 3 (pre++zs.flatMap intWord++tail) (pre.length+(zs.flatMap intWord).length)
        (savedList zs backing) (out++zs.flatMap intWord) total 1) := by
  induction zs generalizing pre backing out pos with
  | nil =>
  ...
  | cons z zs ih =>
    ...
    rw [hf,hs] at hb
    have ht := ih (pre++intWord z) (saved z backing) (out++intWord z) (pos+1)
...
theorem list_run (pre tail backing out : List Bool) (zs : List ℤ) :
    ∃ r,runFrom loop ((zs.flatMap intWord).length+6*zs.length+3)
      ...
      r.final=cfg 3 (pre++zs.flatMap intWord++tail) (pre.length+(zs.flatMap intWord).length)
        (savedList zs backing) (out++zs.flatMap intWord) zs.length 1 ∧
...
end NearCubicWires.RepairOrdinary.DecompositionSource.Fields
