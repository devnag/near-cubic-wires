import Proof.PCP.PCPPNativeOracleLoop

/-! Append the actual shared negation following one copied oracle bank.
This is the single NOT node prescribed by copyQuery; its child address is
computed by the retained raw base/original-output counters. -/
...
def firstBits : List Bool := natWord 2
...
theorem emitted_nodes (r base index : ℕ) :
    emitted base index=PCPPRequestNodeSchema.native (.not (PCPPSubstitution.address base index) : BooleanNode r) := by
