...
/-! The binary-node caller retains both actual raw child indices. Its
second address call shares exactly the first call's reusable workspace. -/
...
def data (base left right C : ℕ) (out : List Bool) : Fin 25 → List Bool :=
  Fin.addCases (motive := fun _ : Fin 25 => List Bool)
    (PCPPNativeAddressReusable.data base left C out) (fun _ : Fin 1 => List.replicate right true)
def heads (out : List Bool) : Fin 25 → ℕ :=
  Fin.addCases (motive := fun _ : Fin 25 => ℕ) (PCPPNativeAddressReusable.heads out) (fun _ : Fin 1 => 0)
...
noncomputable def rightMachine := RecoveryFocus.machine rightSlots PCPPNativeAddressReusable.machine
...
theorem right_input (base left right C : ℕ) (out : List Bool) (i : Fin 24) :
    heads out (rightSlots i)=PCPPNativeAddressReusable.heads out i ∧
      data base left right C out (rightSlots i)=PCPPNativeAddressReusable.data base right C out i := by
  fin_cases i <;> simp [heads,data,rightSlots,PCPPNativeAddressReusable.heads,
    PCPPNativeAddressReusable.data,Fin.addCases]

theorem right_run (base left right C : ℕ) (out : List Bool)
    (hC : PCPPNativeAddressAppend.budget base right+1 ≤ C) :
    ∃ r,runFrom rightMachine (PCPPNativeAddressReusable.budget base right C)
      (rightEntry base left right C out)=some r ∧
      r.steps ≤ PCPPNativeAddressReusable.budget base right C ∧
      r.final.heads=heads (out++natWord (PCPPSubstitution.address base right)) ∧
      r.final.tapes=data base left right C (out++natWord (PCPPSubstitution.address base right)) := by
  obtain ⟨raw,hr,rs,rh,rt⟩ := PCPPNativeAddressReusable.append_run base right C out hC
  obtain ⟨r,hrun,_,steps,hh,ht,keep⟩ := RecoveryFocus.dock rightSlots right_injective
    PCPPNativeAddressReusable.machine _ (heads out) (data base left right C out)
    (PCPPNativeAddressReusable.entry base right C out)
  ...
  have hhead (j : Fin 24) : r.final.heads (rightSlots j)=
      PCPPNativeAddressReusable.heads (out++natWord (PCPPSubstitution.address base right)) j := by
    rw [hh,rh]
  have htape (j : Fin 24) : r.final.tapes (rightSlots j)=
      PCPPNativeAddressReusable.data base right C (out++natWord (PCPPSubstitution.address base right)) j := by
    rw [ht,rt]
