import Proof.PCP.PCPSerializerMassField
...
namespace NearCubicWires.RepairOrdinary.PCPSerializerCapacity.MassLoop
open LocalBitMultitape RepairOrdinary RecoveryExecution RepairSource.VerifierDecoding
open RepairSource.ProjectionNormalization
open FieldList (stream stream_nil stream_cons)
set_option autoImplicit false
set_option maxHeartbeats 250000
set_option maxRecDepth 120000
set_option warningAsError true

noncomputable def machine := RepeatMachine.machine FieldCount.machine (fun _ _ => true)
noncomputable def cfg (phase : Fin 5) (source : List Bool) (pos : ℕ) (out : List Bool)
    (total driver : ℕ) := RepeatMachine.cfg phase (FieldCount.cfg 0 source pos out) total driver
...
theorem remaining (pre : List Bool) (fields : List (List Bool)) (suffix out : List Bool)
    ...
    Timed machine ((stream fields).length+2*fields.length+total+3)
      ...
      (cfg 3 (pre++stream fields++suffix) (pre.length+(stream fields).length)
        (out++List.replicate (stream fields).length true) total 1) := by
  induction fields generalizing pre out pos with
  | nil =>
    have hp : pos=total := by simpa using hn
    subst pos
    simpa only [machine,cfg,stream_nil,List.append_nil,List.length_nil,List.replicate_zero,Nat.mul_zero,Nat.add_zero,Nat.zero_add]
      using RepeatMachine.exhaust FieldCount.machine (fun _ _ => true)
        (FieldCount.cfg 0 (pre++suffix) pre.length out) total
  | cons bits fields ih =>
    obtain ⟨r,hr,hf,hs⟩ := FieldCount.count_run pre bits (stream fields++suffix) out
    have hstep := RepeatMachine.iteration FieldCount.machine (fun _ _ => true)
      (FieldCount.cfg 0 (pre++frame bits++(stream fields++suffix)) pre.length out)
      total pos r (by rfl) (by simp only [List.length_cons] at hn; omega) hr
    rw [hf,hs] at hstep
    have htail := ih (pre++frame bits) (out++List.replicate (2*bits.length+1) true) (pos+1)
    ...
    have hmid : cfg 0 (pre++frame bits++(stream fields++suffix))
        (pre.length+2*bits.length+1) (out++List.replicate (2*bits.length+1) true) total (pos+2)=
      cfg 0 ((pre++frame bits)++stream fields++suffix)
        (pre++frame bits).length (out++List.replicate (2*bits.length+1) true) total ((pos+1)+1) := by
    ...
    change Timed machine ((2*bits.length+1)+2)
      ...
      (cfg 0 (pre++frame bits++(stream fields++suffix))
        (pre.length+2*bits.length+1) (out++List.replicate (2*bits.length+1) true) total (pos+2)) at hstep
    rw [hmid] at hstep
    ...
    have htime : (2*bits.length+1+2)+((stream fields).length+2*fields.length+total+3)=
      ...
      omega
    rw [htime] at hall
    simpa only [stream_cons,List.append_assoc,List.length_append,hlen,Nat.add_assoc,
      List.replicate_add] using hall

theorem count_run (pre : List Bool) (fields : List (List Bool)) (suffix out : List Bool) :
    ∃ r,runFrom machine ((stream fields).length+3*fields.length+3)
      ...
      r.final=cfg 3 (pre++stream fields++suffix) (pre.length+(stream fields).length)
        (out++List.replicate (stream fields).length true) fields.length 1 ∧
...
end NearCubicWires.RepairOrdinary.PCPSerializerCapacity.MassLoop
