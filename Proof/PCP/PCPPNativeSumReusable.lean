import Proof.PCP.PCPPNativeSumReset

/-! The raw-natural sum append operation with physically reusable workspace.
The bounded scratch is swept after its heads are reset; the two actual summands
and live descriptor cursor are retained across the whole operation. -/
namespace NearCubicWires.RepairOrdinary.PCPPNativeSumReusable
...
noncomputable def first := RecoveryFocus.machine resetSlots PCPPNativeSumReset.machine
...
def budget (base index C : ℕ) := 2*PCPPNativeSumAppend.budget base index+2*C+7

theorem reset_input (base index C : ℕ) (out : List Bool) (i : Fin 22) :
    heads out (resetSlots i)=(PCPPNativeSumReset.entry base index C out).heads i ∧
      data base index C out (resetSlots i)=(PCPPNativeSumReset.entry base index C out).tapes i := by
  fin_cases i <;> simp [heads,data,resetSlots,PCPPNativeSumReset.entry,PCPPNativeSumReset.caps,
    PCPPNativeSumAppend.entry,PCPPNativeSumAppend.heads,PCPPNativeSumAppend.data,
...
theorem append_run (base index C : ℕ) (out : List Bool)
    ...
    ∃ r,runFrom machine (budget base index C) (entry base index C out)=some r ∧
      ...
      r.final.heads=heads (out++natWord ((base+index))) ∧
      r.final.tapes=data base index C (out++natWord ((base+index))) := by
  obtain ⟨reset,hr,rs,rt,rh,r0,rh0,r1,rh1,work⟩ := PCPPNativeSumReset.reset_run base index C out hC
  obtain ⟨a,ha,_,asteps,aheads,atapes,akeep⟩ := RecoveryFocus.dock
    ...
    PCPPNativeSumReset.machine _ (heads out) (data base index C out)
    (PCPPNativeSumReset.entry base index C out)
  ...
  have timeEq : 2*PCPPNativeSumAppend.budget base index+2+1+(2*C+4)=budget base index C := by
    unfold budget; omega
  rw [timeEq] at joined
...
end NearCubicWires.RepairOrdinary.PCPPNativeSumReusable
