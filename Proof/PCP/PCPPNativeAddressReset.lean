import Proof.PCP.PCPPNativeAddressAppend

/-! Reset only the native address appender's bounded local heads. The live
descriptor cursor and the two retained input counters are excluded. -/
namespace NearCubicWires.RepairOrdinary.PCPPNativeAddressReset
...
noncomputable def machine := MaskedReset.machine PCPPNativeAddressAppend.machine selected
def caps (C : ℕ) (i : Fin 22) : ℕ := if 2 ≤ i.val ∧ i.val < 20 ∨ i.val=21 then C else 0
noncomputable def entry (base index C : ℕ) (out : List Bool) :=
  ZeroPadding.config (caps C) (Rewind.recording (PCPPNativeAddressAppend.entry base index out) 0)

theorem local_initial (base index : ℕ) (out : List Bool) (i : Fin 21)
    (hi : 2 ≤ i.val ∧ i.val < 20) :
    (PCPPNativeAddressAppend.entry base index out).heads i=0 ∧
      (PCPPNativeAddressAppend.entry base index out).tapes i=[] := by
  ...
  simp only [PCPPNativeAddressAppend.entry,PCPPNativeAddressAppend.data,
    PCPPNativeAddressAppend.heads,h0,h1,h20,ite_false,and_self]

theorem reset_run (base index C : ℕ) (out : List Bool)
    (hC : PCPPNativeAddressAppend.budget base index+1 ≤ C) :
    ∃ r,runFrom machine (2*PCPPNativeAddressAppend.budget base index+2)
      (entry base index C out)=some r ∧
      r.steps ≤ 2*PCPPNativeAddressAppend.budget base index+2 ∧
      r.final.tapes 20=out++RepairRepresentation.natWord (PCPPSubstitution.address base index) ∧
      r.final.heads 20=(out++RepairRepresentation.natWord (PCPPSubstitution.address base index)).length ∧
  ...
  obtain ⟨raw,hr,rs,rt,rh,r0,rh0,r1,rh1⟩ := PCPPNativeAddressAppend.address_append_run base index out
  ...
  obtain ⟨reset,hreset,rf,rsteps,_⟩ := MaskedReset.reset_run
    PCPPNativeAddressAppend.machine selected _ _ raw hr hh
  ...
  have runBound := runFrom_moreFuel machine _
    (2*PCPPNativeAddressAppend.budget base index+2-(2*raw.steps+2)) _ result hresult
  rw [Nat.add_sub_of_le (by omega : 2*raw.steps+2 ≤
    2*PCPPNativeAddressAppend.budget base index+2)] at runBound
  ...
  · intro i hi
    ...
    · let j : Fin 21 := ⟨i.val,by omega⟩
      ...
      have ht := PCPSerializerReuse.tape_support PCPPNativeAddressAppend.machine _ _ raw hr j 0 0
...
end NearCubicWires.RepairOrdinary.PCPPNativeAddressReset
