...
/-- The family step's append words at call `j` (`_hrefill`'s `outA`'s `app` part). -/
abbrev appTOf {Atom : Type} {U : Nat} (b : Nat) (vd : RCFive.Source.CallValues Atom U) (j : Nat) :=
  CloseoutFinalC10AppendPositioning.tapes b (vd.D j) (vd.logSize j) (vd.resetSize j)
    ⟨vd.coefficient j, vd.total j, vd.denominator j⟩
    ((vd.phasePrefix ++ vd.entries.take j) ++ [(⟨vd.coefficient j, vd.total j, vd.denominator j⟩ : Stream.Entry)])
