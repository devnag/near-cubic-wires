...
import Proof.SourceAssembly.SourceSkelFirstStartV
...
/-- The chain's view already holds the call's family input on the slots. -/
theorem chainView_install_slots {mask : MaskProducer} {selector : CyclicChoice.Laws}
    ...
    (vd : RCFive.Source.CallValues Atom code.sourceTapes) (M j : Nat) (X : Fin code.sourceTapes → List Bool) :
    install code.slots (chainView code b vd M j X) (inTOf code b vd j) = chainView code b vd M j X := by
  unfold chainView viewOf
  split_ifs <;> exact install_install code.slots (slots_injective code) _ _
