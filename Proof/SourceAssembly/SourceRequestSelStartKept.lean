...
namespace NearCubicWires.SourceRequest.SelStartKept
...
/-! ## `castSucc` transport of the site's kept set and kept words -/

theorem KSite_castSucc (V : ℕ) (hV : (𝔡).U ≤ V) (hV1 : (𝔡).U ≤ V + 1) (mode : Bool) (y : Fin V)
    (hy : KSite selector xtra mask packets rows sources gamma hg hh p V hV mode y) : KSite selector xtra mask packets rows sources gamma hg hh p (V + 1) hV1 mode y.castSucc := by
  ...
  · left
    ...
    · right; right; left
      exact ⟨i, Fin.ext (by rw [cacheSite_val, Fin.val_castSucc, ← hi, cacheSite_val])⟩
  ...
  · right; right; right; right; right
    exact Fin.ext (by rw [Fin.val_castSucc, h]; rfl)

theorem K0Site_castSucc (V : ℕ) (hV : (𝔡).U ≤ V) (hV1 : (𝔡).U ≤ V + 1) (mode : Bool) (frameW qW : List Bool)
    (cdW : Fin 19 → List Bool) (resW : ℕ → List Bool) (Vv NC : ℕ) (y : Fin V) :
    K0Site selector xtra mask packets rows sources gamma hg hh p (V + 1) hV1 mode frameW qW cdW resW Vv NC y.castSucc = K0Site selector xtra mask packets rows sources gamma hg hh p V hV mode frameW qW cdW resW Vv NC y := by
  by_cases hc : ∃ i, cacheSite selector xtra mask packets rows sources gamma hg hh p V hV mode i = y
  · obtain ⟨i, rfl⟩ := hc
    have e : (cacheSite selector xtra mask packets rows sources gamma hg hh p V hV mode i).castSucc = cacheSite selector xtra mask packets rows sources gamma hg hh p (V + 1) hV1 mode i :=
      Fin.ext (by rw [Fin.val_castSucc, cacheSite_val, cacheSite_val])
    rw [e, K0Site_cache, K0Site_cache]
  · have hx : ∀ i, (PCJda54a286946142d3_BranchPhases.cache sources p 𝔨 𝔯 𝔰 mode i).val ≠ y.val := fun i h => hc ⟨i, Fin.ext (by rw [cacheSite_val]; exact h)⟩
    rw [K0Site_off selector xtra mask packets rows sources gamma hg hh p (V + 1) hV1 mode frameW qW cdW resW Vv NC y.castSucc (by simpa using hx),
      K0Site_off selector xtra mask packets rows sources gamma hg hh p V hV mode frameW qW cdW resW Vv NC y hx]
    simp only [Fin.val_castSucc]
    by_cases ht : y = terminalSite selector xtra mask packets rows sources gamma hg hh p V hV
    · have ht1 : y.castSucc = terminalSite selector xtra mask packets rows sources gamma hg hh p (V + 1) hV1 := Fin.ext (by rw [Fin.val_castSucc, ht]; rfl)
      simp only [eq_true ht1, eq_true ht, if_true]
    · have ht1 : ¬ y.castSucc = terminalSite selector xtra mask packets rows sources gamma hg hh p (V + 1) hV1 := fun h => ht (Fin.ext (by
        have := congrArg Fin.val h; rw [Fin.val_castSucc] at this; rw [this]; rfl))
      simp only [eq_false ht1, eq_false ht, if_false]

end site

end
end NearCubicWires.SourceRequest.SelStartKept
end
