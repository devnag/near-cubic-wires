import Proof.SourceAssembly.SourceRequestSelHG7Final
...
namespace NearCubicWires.SourceRequest.SelHG7Table
...
theorem hG_site (den : Nat) (hden : 0 < den) (n : Nat) (x : BitInput n) (bits : List Bool)
    ...
    (ph : Phase) (ci : Fin NCC) (j : ℕ) :
    NearCubicWires.SourceRequest.SelG7Spec.g7costW sources mask MBC (RepairSource.CloseoutLanguage.selectedPCPP sources) (req sources 𝔨 (PolynomialClock.ordinaryClock 𝔨) x oracleC) ci coordC bits ph 𝔏 tgtC cwidC cwCq DCq 𝔟 PwC WCq LdCq modeC j ≤
      RuntimeShape.tableClass 𝔏 0 𝔮𝔰 := by
  have hden1 : 1 ≤ den := hden
  obtain ⟨hr, hqm, hL⟩ := SourceSteps.site_call_facts sources p den hden 𝔨 𝔯 𝔰 n x bits hp ph ci 𝔏 hden1 hN j
  exact SourceRequest.SelG7Final.g7cost_final selector xtra mask packets rows sources gamma hg hh p 𝔏 tgtC (SourceBudget.tgt sources p) den n hon x bits
    (SourceRequest.AdmitRead.hlen_site sources p den hden 𝔨 x bits hp) modeC ph ci hden1 j hr hqm hL

end site

end
end NearCubicWires.SourceRequest.SelHG7Table
end
