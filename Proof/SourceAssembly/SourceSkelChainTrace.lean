...
theorem traceOfChain (mask : MaskProducer) (selector : CyclicChoice.Laws)
  ...
  exact traceOf mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old
    (fun j => (callChain N (fun _ => 0) (code ph).slots (inTOf (code ph) (C10PartsSchedule.entryWidthSchedule sources k r n) (clauseVals mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old Hd Ad Rc familyCost refillCost firstCost counterReserve)) Inv (goodOf (code ph) (C10PartsSchedule.entryWidthSchedule sources k r n) (clauseVals mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old Hd Ad Rc familyCost refillCost firstCost counterReserve)) seam H0c A0c j).1)
    (fun j => (callChain N (fun _ => 0) (code ph).slots (inTOf (code ph) (C10PartsSchedule.entryWidthSchedule sources k r n) (clauseVals mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old Hd Ad Rc familyCost refillCost firstCost counterReserve)) Inv (goodOf (code ph) (C10PartsSchedule.entryWidthSchedule sources k r n) (clauseVals mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old Hd Ad Rc familyCost refillCost firstCost counterReserve)) seam H0c A0c j).2)
    Rc familyCost refillCost firstCost counterReserve H0 A0 siteFuel vv
    (hv.trans (chainVals_clauseVals mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old Hd Ad Rc familyCost refillCost
      firstCost counterReserve (C10PartsSchedule.entryWidthSchedule sources k r n) N Inv seam H0c A0c))
    hdeg hC hD hcap hlog h_hw h_hfit
    (hH_of_chain (code ph) (C10PartsSchedule.entryWidthSchedule sources k r n) (clauseVals mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old Hd Ad Rc familyCost refillCost firstCost counterReserve) seam H0c A0c h0 hN hH0 vv hv)
    (hA_of_chain (code ph) (C10PartsSchedule.entryWidthSchedule sources k r n) (clauseVals mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old Hd Ad Rc familyCost refillCost firstCost counterReserve) seam H0c A0c h0 hN hA0 vv hv)
    h_hold h_hr h_heH h_haH h_hfields h_hcost
    (hrefill_of_chain (code ph) (C10PartsSchedule.entryWidthSchedule sources k r n) (clauseVals mask selector packets rows compiler sources p den hden k r scratch n x bits hp site code ph ci L lay deg V dflt Dw capw logw resetw old Hd Ad Rc familyCost refillCost firstCost counterReserve) seam H0c A0c h0 hN vv hv)
    h_hfirst h_hcode hbudget
