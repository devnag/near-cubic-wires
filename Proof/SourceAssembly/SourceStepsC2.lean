...
open Classical in
/-- The two-level choice's specification. -/
theorem choose2_spec {α β : Type} {P : α → β → Prop} (h : ∃ a b, P a b) (d : α × β) :
    P (if h' : ∃ a b, P a b then (Classical.choose h', Classical.choose (Classical.choose_spec h')) else d).1
      (if h' : ∃ a b, P a b then (Classical.choose h', Classical.choose (Classical.choose_spec h')) else d).2 := by
  rw [dif_pos h]
