...
theorem c2Hyps5_mk (Et : EntrySite (ControllerSelectedContinuation.bodyTapes sources p k r (scratchOf mask packets rows sources res)))
    (EF : Phase → Fin NCC → List Stream.Entry) (siteFuel : Nat) {eR : Nat} {eV : Nat} {X : Nat} (pl : Phase → InitRun.Place 𝔇 se.extra sp.extra gW eR eV X ((𝒞).sourceTapes + 1)) (Ce : Nat) (Kc : Fin ((𝒞).sourceTapes + 1) → Prop) (K0e : Phase → Nat → Fin ((𝒞).sourceTapes + 1) → List Bool) (KH0e : Fin ((𝒞).sourceTapes + 1) → Nat) (cnt : Fin ((𝒞).sourceTapes + 1)) (c15 : Fin ((𝒞).sourceTapes + 1)) (q284 : Fin ((𝒞).sourceTapes + 1)) (c17 : Fin ((𝒞).sourceTapes + 1)) (c18 : Fin ((𝒞).sourceTapes + 1))  
    ...
    (hStart : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      entryInvAt4 sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp Et EF ph ci.val A H → ∃ H1 A1, CSP5C H1 A1)
    (hTN : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      TraceNums mask selector packets rows compiler sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp site codeF ph ci L layC
        ...
        (CloseoutFinalC10AppendWorkspaceInit.capacity bS) oldC HdZ AdZ Rc familyCost refillCost firstCost counterReserve H A siteFuel NC5 VV5)
    ...
    (hC : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      CacheHole mask selector packets rows sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp codeF ph ci VV5)
    ...
    (hN : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat), (∃ H1 A1, CSP5C H1 A1) →
    ...
    (hEt : Et = ES) :
    C2Hyps5 mask packets rows sources res hres p k r ph se sp e g7F preFF compiler den hden n x bits hp site L Rc Rk layF K K0F KH0 V dflt familyCost firstCost counterReserve refillCost Et EF siteFuel pl Ce Kc K0e KH0e cnt c15 q284 c17 c18
      (fun ci A H H1 A1 => CSP5C H1 A1) (fun ci A H1 A1 => Inv6C H1 A1) where
  hSP := hSP_5 mask packets rows sources res hres p k r ph se sp e g7F preFF compiler den hden n x bits hp site L Rc Rk layF K K0F KH0 V dflt familyCost firstCost counterReserve refillCost
  hStart := hStart
  hInvX := hInvX_6 mask packets rows sources res hres p k r ph se sp e g7F preFF compiler den hden n x bits hp site L Rc Rk layF K K0F KH0 V dflt familyCost firstCost counterReserve refillCost
  hSeam := hSeam
  h0X := h0X_6 mask packets rows sources res hres p k r ph se sp e g7F preFF compiler den hden n x bits hp site L Rc Rk layF K K0F KH0 V dflt familyCost firstCost counterReserve refillCost
  hTN := hTN
  hsite := hsite
  hK := hK
  hC := hC
  ...
  hpre := hpre
  hEt := hEt
  henc := henc_6 mask packets rows sources res hres p k r ph se sp e g7F preFF compiler den hden n x bits hp site L Rc Rk layF K K0F KH0 V dflt familyCost firstCost counterReserve refillCost hSeam
