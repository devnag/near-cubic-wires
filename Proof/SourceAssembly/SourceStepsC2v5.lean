import Proof.SourceAssembly.SourceStepsC2
...
/-- **C2 v5: THE PHASE'S `ClauseSteps`** (decision 99b), at `Inv := entryInvAt4 … ES EF ph`. As `stepsC`, with the first cycle's exit chosen from
ANY predicate `SP` refining `ChainStartP` (e.g. `ChainStartP5`), so that `hN`/`henc`/`h0X` may read `SP` at the chosen exit. -/
noncomputable def stepsC5 {eR eV X : Nat}
    ...
    (EF : Phase → Fin NCC → List Stream.Entry) (siteFuel : Nat)
    (SP : Fin NCC → (Fin BTS → List Bool) → (Fin BTS → Nat) → (Fin (UU + 1) → Nat) → (Fin (UU + 1) → List Bool) → Prop)
    (hSP : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat) (H1 : Fin (UU + 1) → Nat) (A1 : Fin (UU + 1) → List Bool),
      SP ci A H H1 A1 → CSPC H1 A1)
    (hStart : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      entryInvAt4 sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp ES EF ph ci.val A H → ∃ H1 A1, SP ci A H H1 A1)
    (InvX : Fin NCC → (Fin BTS → List Bool) → (Fin (UU + 1) → Nat) → (Fin (UU + 1) → List Bool) →
      Nat → (Fin UU → Nat) → (Fin UU → List Bool) → Prop)
    (hInvX : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H1 : Fin (UU + 1) → Nat) (A1 : Fin (UU + 1) → List Bool) j Hc Ac,
      InvX ci A H1 A1 j Hc Ac → Inv5C H1 A1 j Hc Ac)
    (hSeam : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H1 : Fin (UU + 1) → Nat) (A1 : Fin (UU + 1) → List Bool),
      SeamSpec NC5 (fun _ => 0) (𝒞).slots (inTOf 𝒞 bS vdC) (InvX ci A H1 A1) (goodOf 𝒞 bS vdC))
    (h0X : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat), (∃ H1 A1, SP ci A H H1 A1) →
      InvX ci A (startC).1 (startC).2 0 H0C A0C)
    (hTN : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      TraceNums mask selector packets rows compiler sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp site codeF ph ci L layC
        (degOf sources selector coordC ph ci L tgtC modeC layC) V dflt (InitRun.D0 bS) (InitRun.cap0 bS) (CloseoutFinalC10AppendWorkspaceInit.capacity bS)
        (CloseoutFinalC10AppendWorkspaceInit.capacity bS) oldC HdZ AdZ Rc familyCost refillCost firstCost counterReserve H A siteFuel NC5 VVC)
    (hsite : site modeC ph = SourceParent.siteOfCode 𝒞)
    (hK : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      KeepHole mask selector packets rows sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp codeF ph ci H A VVC)
    (hC : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      CacheHole mask selector packets rows sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp codeF ph ci VVC)
    (hH : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      HappHole mask selector packets rows sources p k r (scratchOf mask packets rows sources res) n x bits codeF ph A VVC)
    (hN : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat), (∃ H1 A1, SP ci A H H1 A1) →
      NextHole mask selector packets rows compiler sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp site codeF ph ci H A VVC
        𝔇 pl e 𝒽 Rc Rk Ce Kc K0e KH0e cnt c15 q284 c17 c18 bS vQ vMB vMS vWS vRW vBF vU0 EF K K0C KH0 (fuelOf 𝒞 bS vdC NC5))
    (hcR : counterReserve = Rc) (hNR : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool), NC5 + 2 ≤ Rc)
    (hwR : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool), (CompareMachine.word NC5).length ≤ Rc) (hcnt : cnt = Fin.last (𝒞).sourceTapes)
    ...
    (hEF : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool), (vdC).entries = EF ph ci)
    (hpre : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool), (vdC).phasePrefix = prefixEntries (EF ph) ci.val)
    (henc : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat), (∃ H1 A1, SP ci A H H1 A1) →
      EncWords 𝒽 Rc bS ((VVC).H NC5) ((VVC).A NC5)) :
    ClauseSteps mask selector packets rows compiler sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp site codeF ph siteFuel
      (fun c A2 H2 => entryInvAt4 sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp ES EF ph c A2 H2) :=
  stepsOf (mask := mask) (selector := selector) (packets := packets) (rows := rows) (compiler := compiler) (sources := sources) (p := p)
    (den := den) (hden := hden) (k := k) (r := r) (scratch := scratchOf mask packets rows sources res) (n := n) (x := x) (bits := bits) (hp := hp)
    (site := site) (code := codeF) ph L layF (fun ci => degOf sources selector coordC ph ci L tgtC modeC layC) V dflt
    (InitRun.D0 bS) (InitRun.cap0 bS) (CloseoutFinalC10AppendWorkspaceInit.capacity bS) (CloseoutFinalC10AppendWorkspaceInit.capacity bS)
    (fun A H ci => oldC) (fun A H ci => (VVC).H) (fun A H ci => (VVC).A) Rc familyCost refillCost firstCost counterReserve
    (fun ci A H hI =>
      have hex : ∃ H1 A1, SP ci A H H1 A1 := hStart ci A H hI
      parts_S mask packets rows sources res hres p k r ph se sp e g7F preFF compiler den hden n x bits hp site ci L Rc Rk layC K K0C KH0 V dflt HdZ AdZ
        familyCost firstCost counterReserve refillCost A H (startC).1 (startC).2 (hSP ci A H _ _ (choose2_spec hex _))
        pl Ce Kc K0e KH0e cnt c15 q284 c17 c18 EF hI siteFuel (InvX ci A (startC).1 (startC).2) (hInvX ci A (startC).1 (startC).2) seamC
        (h0X ci A H hex) (hTN ci A H) hsite (hK ci A H) (hC ci A H) (hH ci A H) (hN ci A H hex) hcR (hNR ci A) (hwR ci A) hcnt (hcacheK ci) (hKc1 ci) (hKc2 ci)
        hKlast h15 h17 h18 hq284F hq284c (hq284 ci) (hEF ci A) (hpre ci A) (henc ci A H hex))
