...
structure C2Hyps5 (Et : EntrySite (ControllerSelectedContinuation.bodyTapes sources p k r (scratchOf mask packets rows sources res)))
    (EF : Phase → Fin NCC → List Stream.Entry) (siteFuel : Nat) {eR : Nat} {eV : Nat} {X : Nat} (pl : Phase → InitRun.Place 𝔇 se.extra sp.extra gW eR eV X ((𝒞).sourceTapes + 1)) (Ce : Nat) (Kc : Fin ((𝒞).sourceTapes + 1) → Prop) (K0e : Phase → Nat → Fin ((𝒞).sourceTapes + 1) → List Bool) (KH0e : Fin ((𝒞).sourceTapes + 1) → Nat) (cnt : Fin ((𝒞).sourceTapes + 1)) (c15 : Fin ((𝒞).sourceTapes + 1)) (q284 : Fin ((𝒞).sourceTapes + 1)) (c17 : Fin ((𝒞).sourceTapes + 1)) (c18 : Fin ((𝒞).sourceTapes + 1)) (SP : Fin NCC → (Fin BTS → List Bool) → (Fin BTS → Nat) → (Fin (UU + 1) → Nat) → (Fin (UU + 1) → List Bool) → Prop) (InvX : Fin NCC → (Fin BTS → List Bool) → (Fin (UU + 1) → Nat) → (Fin (UU + 1) → List Bool) →
      Nat → (Fin UU → Nat) → (Fin UU → List Bool) → Prop) : Prop where
  ...
  hStart : ∀ (ci : Fin NCC) (A : Fin BTS → List Bool) (H : Fin BTS → Nat),
      entryInvAt4 sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp Et EF ph ci.val A H → ∃ H1 A1, SP ci A H H1 A1
...
/-- **C2 v5 from its hypotheses**: the phase's `ClauseSteps` at the target entry site `Et`. -/
noncomputable def stepsC5_of (Et : EntrySite (ControllerSelectedContinuation.bodyTapes sources p k r (scratchOf mask packets rows sources res)))
    (EF : Phase → Fin NCC → List Stream.Entry) (siteFuel : Nat) {eR : Nat} {eV : Nat} {X : Nat} (pl : Phase → InitRun.Place 𝔇 se.extra sp.extra gW eR eV X ((𝒞).sourceTapes + 1)) (Ce : Nat) (Kc : Fin ((𝒞).sourceTapes + 1) → Prop) (K0e : Phase → Nat → Fin ((𝒞).sourceTapes + 1) → List Bool) (KH0e : Fin ((𝒞).sourceTapes + 1) → Nat) (cnt : Fin ((𝒞).sourceTapes + 1)) (c15 : Fin ((𝒞).sourceTapes + 1)) (q284 : Fin ((𝒞).sourceTapes + 1)) (c17 : Fin ((𝒞).sourceTapes + 1)) (c18 : Fin ((𝒞).sourceTapes + 1)) (SP : Fin NCC → (Fin BTS → List Bool) → (Fin BTS → Nat) → (Fin (UU + 1) → Nat) → (Fin (UU + 1) → List Bool) → Prop) (InvX : Fin NCC → (Fin BTS → List Bool) → (Fin (UU + 1) → Nat) → (Fin (UU + 1) → List Bool) →
      Nat → (Fin UU → Nat) → (Fin UU → List Bool) → Prop)
    (h : C2Hyps5 mask packets rows sources res hres p k r ph se sp e g7F preFF compiler den hden n x bits hp site L Rc Rk layF K K0F KH0 V dflt
      familyCost firstCost counterReserve refillCost Et EF siteFuel pl Ce Kc K0e KH0e cnt c15 q284 c17 c18 SP InvX) :
    ClauseSteps mask selector packets rows compiler sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp site codeF ph siteFuel
      (fun c A2 H2 => entryInvAt4 sources p den hden k r (scratchOf mask packets rows sources res) n x bits hp Et EF ph c A2 H2) := by
  rw [h.hEt]
  exact stepsC5 mask packets rows sources res hres p k r ph se sp e g7F preFF compiler den hden n x bits hp site L Rc Rk layF K K0F KH0 V dflt
    familyCost firstCost counterReserve refillCost pl Ce Kc K0e KH0e cnt c15 q284 c17 c18 EF siteFuel SP h.hSP (fun ci A H hI => h.hStart ci A H (h.hEt ▸ hI)) InvX h.hInvX h.hSeam h.h0X h.hTN h.hsite h.hK h.hC h.hH h.hN h.hcR h.hNR h.hwR h.hcnt h.hcacheK h.hKc1 h.hKc2 h.hKlast h.h15 h.h17 h.h18 h.hq284F h.hq284c h.hq284 h.hEF h.hpre h.henc
